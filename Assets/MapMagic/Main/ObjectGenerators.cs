		...
		public override void Generate (MapMagic.Chunk chunk)
		{
			...
			foreach (SpatialObject obj in objs.AllObjs())
			{
					...
					for (int z=min.z; z<max.z; z++)
				{
					...
					float val = stamp.CheckGet((int)(relativePos.x*stamp.rect.size.x), (int)(relativePos.y*stamp.rect.size.z)); //TODO use bilenear filtering
	...
	public class FlattenGenerator : Generator
	{
		...
		public override void Generate (MapMagic.Chunk chunk)
		{
			...
			foreach (SpatialObject obj in objects.AllObjs())
			{
				...
				BlobGenerator.DrawBlob(dst, obj.pos, 1, curRadius, fallof, noiseAmount, noiseSize);
			}
