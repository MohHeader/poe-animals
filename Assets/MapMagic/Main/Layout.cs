	...
	public class Layout
	{
		...
		#region Icon
			...
			public bool Icon (Texture2D texture, Rect rect, IconAligment horizontalAlign=IconAligment.resize, IconAligment verticalAlign=IconAligment.resize, bool tile=false, bool clickable=false)
			{
				...
				//drawing texture
				if (!tile) GUI.DrawTexture(ToDisplay(rect), texture); 
				else
				{
					//Debug.Log(zoom);
		...
		#region Fields
			...
			{ src = Field<T> (src, label, rect, min, max, fieldSize, sliderSize, monitorChange, useEvent, disabled, dragChange, slider, quadratic, allowSceneObject, style, tooltip); }
				...
				string tooltip=null)
			{
				...
				useEvent.Verify(this.useEvent); disabled.Verify(this.disabled);
			...
			//	GUI.SetNextControlName("LayoutField");
				...
				else if (type == typeof(bool)) 
				{
					...
					if (zoom > 0.75f) dstObj = UnityEditor.EditorGUI.Toggle(ToDisplay(fRect), (bool)srcObj);
				...
				else if (type.IsSubclassOf(typeof(UnityEngine.Object))) dstObj = UnityEditor.EditorGUI.ObjectField(fieldRect, (UnityEngine.Object)srcObj, type, allowSceneObject);
				#endif
			...
			public void Curve (AnimationCurve src, Rect rect, float min=-200000000, float max=200000000, Color color=new Color(), string tooltip=null)
			{
				...
				if (UnityEditor.EditorGUI.EndChangeCheck()) SetChange(true);
				else SetChange(false);
				#endif
			}
				...
				string tooltip=null )
			{
				...
				bool result = GUI.Button(ToDisplay(rect), content);
		...
		#region Layered
			...
			public void DrawLayered (ILayered splatOut, string label="", string tooltip="")
			{
				//array buttons
				Par();
				...
				if (Button(rect:Inset(0.15f), tooltip:"Remove element") &&
					splatOut.selected < splatOut.layers.Length &&
					!splatOut.layers[splatOut.selected].pinned)
				{
					if (OnBeforeChange != null) OnBeforeChange();
				...
				if (Button(rect:Inset(0.15f), tooltip:"Move selected up") && 
					splatOut.selected < splatOut.layers.Length-1 &&
					!splatOut.layers[splatOut.selected].pinned && !splatOut.layers[splatOut.selected+1].pinned) 
				{ 
					if (OnBeforeChange != null) OnBeforeChange();
				...
				if (splatOut.selected < splatOut.layers.Length)
				{
					...
					int selectedLine = splatOut.layers.Length-1 - splatOut.selected; //num inversed from bottom
...
//					if (UnityEditor.EditorGUIUtility.isProSkin) DrawIcon(ToBase(backgroundRect), "MapMagic_LayerBackground_pro");
				...
				//for (int i=0; i<splatOut.layers.Length; i++)
				for (int i=splatOut.layers.Length-1; i>=0; i--)
				{
					ILayer layer = splatOut.layers[i];
					bool selected = i==splatOut.selected;
					...
					if (!selected) 
					{
						...
						if (Event.current.type == EventType.MouseDown && selectRect.Contains(Event.current.mousePosition) ) newSelected = i;
						#endif
				...
				//changing selection
				if (splatOut.selected != newSelected)
				{
					#if UNITY_EDITOR
					UnityEditor.EditorGUI.FocusTextInControl(""); //stop writing on selection change
					#endif
